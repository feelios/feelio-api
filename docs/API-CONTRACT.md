...
```json
{
  ...
  "encouragement": "스트레스받은 날이 많았네, 이번 주는 배달 대신 산책 한 번 어때?",
  "status": "WARNING",
  "emotion": "스트레스"
}
```
...
- AI 비활성화·실패·타임아웃·빈 응답이면 규칙기반 문장으로 채워 응답한다. **감정 개인화는 규칙기반 폴백에서도 유지된다.** `evaluation`·`encouragement`는 항상 비어 있지 않으며 API는 200 성공한다.
