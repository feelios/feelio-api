...
AI를 쓰는 응답은 **어떤 경우에도 500을 내지 않는다.** 문구만 규칙기반으로 바뀌고 200으로 나간다.

- **호출 단위 타임아웃**: `openai.timeout-seconds` (기본 4초).
  §1 상 프론트가 5초에 요청을 포기하므로 서버 대기는 그 안에 들어와야 한다.
  더 길게 잡으면 폴백 문장이 만들어져도 사용자에게 닿지 못한다.
- **서킷 브레이커**: 연속 실패가 `feelio.ai.circuit.failure-threshold`(기본 3)에 닿으면
  `feelio.ai.circuit.open-seconds`(기본 60) 동안 호출을 아예 건너뛰고 곧바로 규칙기반으로 답한다.
  외부 API가 죽어 있을 때 요청마다 타임아웃을 꽉 채우지 않기 위함이다.
  열린 시간이 지나면 한 번 시도해 보고, 성공하면 닫고 실패하면 다시 연다.
- **캐시**: 이번 달 인사이트는 `feelio.insight.ttl-hours`(기본 6) 안에서 재사용하고,
  지난 달 이전은 영구 캐시한다. 대부분의 요청은 AI를 타지 않는다.

> ⚠️ `GET /api/analysis/ai-report` 는 캐시가 만료된 첫 요청에서 생성기를 **순차로 여러 번** 부른다.
> 호출마다 타임아웃이 따로 걸려 최악의 경우 합이 프론트의 5초를 넘는다. 서킷이 열려 있으면 즉시
> 폴백이라 문제가 없지만, 첫 실패 구간은 여전히 느릴 수 있다. 병렬화·요청 단위 예산은 별도 과제다.
...
### GET /api/analysis/budget · 인증 필요

카테고리별 이번 달 예산. `data.budgetItems[]` 는 `categoryName` · `dominantEmotion` · `currentAmount`(이번 달 지출) · `prevAmount`(전월 지출) · `budget`.

예산 산출 규칙 (#196):

1. **필요 저축액** = 진행 중(`ACTIVE`) 목표들의 월 환산액 합. 남은 금액 ÷ 마감까지 남은 개월(최소 1).
2. **고정지출**(`is_fixed`)은 깎지 않는다. `max(전월, 이번 달)` 을 그대로 예산으로 준다 — 월세가 올랐는데 전월 값을 고집하면 첫날부터 초과로 뜬다.
3. **예산 제외 항목**(`is_budgetable = false`)은 목록에도, 삭감률 분모에도 넣지 않는다.
4. **목록에는 이번 달 지출이 있는 카테고리만 넣는다.** 전월에만 있던 카테고리는 줄을 만들지 않는다.
   - 예전에는 전월 기록만 있으면 `currentAmount: 0` 인 줄을 만들었다. 화면에는 `0원 · 0% · 목표 N원`
     으로 남는데 `dominantEmotion` 은 이번 달 기록에서 오므로 말랑이 자리가 비어 라벨만 놓였다.
     아직 쓰지 않은 카테고리라 조정할 것도 알려줄 것도 없다.
   - 전월 통계 자체는 계속 쓴다. 예산의 기준이자 삭감 대상을 고르는 근거다.
5. **삭감은 늘어난 항목에만 배분한다.** 전전월 대비 지출이 늘어난 변동 카테고리만 대상이다.
   - 모든 변동 카테고리를 같은 비율로 깎으면, 전월에 2만 원 쓴 카페까지 20% 깎여 4천 원이 빠진다.
     줄일 여지가 없는 항목만 성가시고 정작 여지가 큰 항목은 덜 깎이는 **평균의 함정**이다.
   - 전전월 기록이 없거나(가입 두 달째) 늘어난 항목이 하나도 없으면 변동 카테고리 전체를 대상으로 되돌린다.
     그래도 저축은 해야 하는데 대상이 비면 아무 데서도 못 모은다.
6. **삭감 대상이 아닌 변동 카테고리는 전월액 + 10%.**
   - 전월액을 그대로 주면 똑같이 써도 소진율 100%가 된다. 지난달에 아껴 쓴 항목일수록
     조금만 더 써도 초과로 넘어가, 아낀 달일수록 다음 달이 빡빡해졌다.
     전월과 같게 쓰는 것은 초과가 아니므로 한 숨 얹는다.
7. **예산은 0원이 되지 않는다.** 삭감률 상한 **30%**, 카테고리별 하한 **전월의 50%**.
   - 목표 저축액이 전월 변동지출보다 크면 예전에는 삭감률이 1.0 이 되어 모든 예산이 0원이었다.
     화면은 즉시 '예산 초과'가 되고 사용자가 할 수 있는 게 없다.
   - 상한이 50%였을 때는 한 달 만에 절반을 줄이라는 뜻이라 지킬 수 없는 숫자였다.
     지킬 수 있어야 다음 달에도 보게 되므로 30%로 낮추고 하한은 절반으로 올렸다.
   - 상·하한을 적용해도 못 모으는 목표는 예산을 0으로 만들 게 아니라 따로 알려줄 일이다.
8. 금액은 천 원 단위로 반올림해 내보낸다.
...
- `소비 위험도`: **예산 소진율** 기준 `위험`(90% 이상) · `주의`(70% 이상 90% 미만) · `안전`(70% 미만, 지출 0원 포함).
  ...
  `note`는 **남은 예산 금액**을 등급별 말투로 적는다 — 안전 `"아직 N원 남았어요"` · 주의 `"이제 N원 남았어요"` ·
  위험 `"N원밖에 안 남았어요"`(90% 이상이어도 아직 남은 경우) / `"N원 넘게 썼어요"`(예산 초과). `N`은 천 단위 구분 기호를 넣는다.
  지출 0원이면 `"이번 달 지출 없음"`, 예산 미설정이면 `"목표를 정하면 예산이 잡혀요"`.
  (소진율 %를 쓰지 않는 이유: 같은 화면의 '목표 예산 현황' 카드가 총예산과 함께 이미 소진율을 보여줘 숫자가 중복된다.)
